# -*- coding: utf-8 -*-
#------------------------------------------------------------
# Movies-Online Regex de direct2watch
...
def directwatch(params):
    plugintools.log("[Movies-Online 0.0.1].directwatch "+repr(params))
    url_user = {}
...
def gethttp_headers(pageurl, referer):
    plugintools.log("[Movies-Online 0.0.1].gethttp_headers")
...
def getparams_directwatch(url_user, body):
    plugintools.log("[Movies-Online 0.0.1].getparams_direct2watch " + repr(url_user) )
...
def getfile_directwatch(url_user, decoded, body):
    plugintools.log("[Movies-Online 0.0.1].getfile_directwatch( "+repr(url_user))
    referer = url_user.get("referer")
...
def get_fileserver(decoded, url_user):
    plugintools.log("Movies-Online get_fileserver "+repr(url_user))
...
def getfile_directwatch(url_user, decoded, body):
    plugintools.log("tvspot getfile_directwatch( "+repr(url_user))
    referer = url_user.get("referer")
    req = urllib2.Request(decoded)
    req.add_header('User-Agent','Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/33.0.1750.154 Safari/537.36')
    req.add_header('Referer', referer)
