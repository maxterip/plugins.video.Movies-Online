# -*- coding: utf-8 -*-
#------------------------------------------------------------
# Movies-Online - XBMC Add-on by Juarrox (juarrox@gmail.com)
...
tools = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.latin ototal/resources/tools', ''))
...
def allmyvideos(params):
    plugintools.log("[Movies-Online 0.0.1].allmyvideos " + repr(params))
...
def vidspot(params):
    plugintools.log("[Movies-Online 0.0.1].vidspot " + repr(params))
...
def vk(params):
    plugintools.log("[Movies-Online 0.0.1].vk " + repr(params))
    ...
    if "This video has been removed from public access" in data:
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('Movies-Online', "El archivo ya no está disponible", 3 , art+'icon.png'))
    else:
        ...
        if url_sintax == "/video":
            ...
            if video_max_hd == "0":
                selector = dialog_vk.select('tvspot', ['240'])

            if video_max_hd == "1":
                selector = dialog_vk.select('tvspot', ['240', '360'])

            if video_max_hd == "2":
                selector = dialog_vk.select('tvspot', ['240', '360', '480'])

            if video_max_hd == "3":
                selector = dialog_vk.select('tvspot', ['240', '360', '480', '720'])                      
