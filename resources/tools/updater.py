# -*- coding: utf-8 -*-
#------------------------------------------------------------
# Movies-Online Updater v0.2 (21.10.2014)
...
def check_update(params):
    plugintools.log("[Movies-Online 0.0.1].check_update "+repr(params))
    ...
    for entry in subchannel:
        ...
        if title_low.find("tvspot") >= 0:
            f = open(home + 'default.py', 'r')
            filename = 'plugin.video.Movies-Online-wip-' + last_update + '.zip'
            ...
            if actualizar == instalada :
                plugintools.log("No es necesario actualizar tvspot")
            else:
                # Hay que actualizar, así que mostramos la entrada
                plugintools.log("Actualizar tvspot")                
        ...
        # ¡OJO! Se aloja en /xbmc/addons/plugin.video-Movies-Onlinet/settings/
        ...
        # ¡OJO! Se aloja en /xbmc/addons/plugin.video-tvspot/
...
def update_file(params):
    plugintools.log("[Movies-Online 0.0.1].Update_now "+repr(params))
    ...
    try:
        ...
        pass
        yesno = 0
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('tvspot', "Actualización fallida", 3 , art+'icon.png'))    
    ...
    try:
        ...
        pass
        yesno = 0
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('tvspot', "Error al sobreescribir", 3 , art+'icon.png'))
    ...
    if yesno == 1:
        progreso.close()
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('tvspot', "Actualización completada", 3 , art+'icon.png'))


def update_palco(params):
    plugintools.log("[Movies-Online 0.0.1].update_palco "+repr(params))
    xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('tvspot', "Espere...", 3 , art+'icon.png'))
    ...
    try:      
        ...
        pass
        xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('tvspot', "Error en la descarga", 3 , art+'icon.png'))       
