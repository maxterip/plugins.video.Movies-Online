# -*- coding: utf-8 -*-
#------------------------------------------------------------
# Movies-Online Regex de cast247
...
def castdos(params):
    plugintools.log("[Movies-Online 0.0.1].cast247 "+repr(params))
    url_user = {}
    ...
    if url_user.get("pageurl").find("domain=") <= 0:
        ...
        if body.find("Private Channel!") > 0 :
            xbmc.executebuiltin("Notification(%s,%s,%i,%s)" % ('tvspot', "Canal privado", 3 , art+'icon.png'))
            return 0
        else:
